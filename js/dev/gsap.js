import { a as getHash, n as bodyLockToggle, o as gotoBlock, r as bodyUnlock, t as bodyLockStatus } from "./functions.min.js";
//#region src/components/custom/bookcall/bookcall.js
...
var futureDate = /* @__PURE__ */ new Date();
futureDate.setDate(futureDate.getDate() + 14);
var el = document.getElementById("date");
el.textContent = formatDate(futureDate);
el.setAttribute("datetime", futureDate.toISOString().split("T")[0]);
//#endregion
...
function updateClock(el) {
	...
	setTimeout(() => {
		timeEl.textContent = (/* @__PURE__ */ new Date()).toLocaleTimeString("en-GB", {
			timeZone: timezone,
			hour: "2-digit",
			minute: "2-digit",
			hour12: false
...
function initClocks() {
	document.querySelectorAll("[data-timezone]").forEach((el) => {
		updateClock(el);
		setInterval(() => updateClock(el), 1e3);
	});
}
initClocks();
function updateYear() {
	const el = document.querySelector(".bottom-footer__copy");
	if (!el) return;
	const currentYear = (/* @__PURE__ */ new Date()).getFullYear();
	el.textContent = el.textContent.replace(/\d{4}/, currentYear);
}
updateYear();
var links = document.querySelectorAll(".column-contact__link");
...
links.forEach((link) => {
	link.addEventListener("mouseenter", () => {
		...
		activeLink = link;
		tooltip.textContent = link.dataset.copy;
	...
	link.addEventListener("click", async (e) => {
		...
		try {
			...
			tooltip.textContent = link.dataset.copied;
			clearTimeout(hideTimeout);
			hideTimeout = setTimeout(() => {
				tooltip.classList.remove("active");
				if (activeLink === link) {
...
var _config = {
	...
	overwrite: false,
	delay: 0
}, _suppressOverwrites$1, _reverting$1, _context$2, _bigNum$2 = 1e8, _tinyNum = 1 / _bigNum$2, _2PI = Math.PI * 2, _HALF_PI = _2PI / 4, _gsID = 0, _sqrt$1 = Math.sqrt, _cos$1 = Math.cos, _sin$1 = Math.sin, _isString$2 = function _isString(value) {
	...
	if (!isArray && _isObject$1(snapTo)) {
		...
		if (snapTo.values) {
			...
			if (is2D = !_isNumber$2(snapTo[0])) radius *= radius;
	...
	var a = !v ? _colorLookup.black : _isNumber$2(v) ? [
		v >> 16,
		v >> 8 & _255,
		v & _255
...
var Timeline = /* @__PURE__ */ function(_Animation) {
	...
	_proto2.getChildren = function getChildren(nested, tweens, timelines, ignoreBeforeTime) {
		...
		if (ignoreBeforeTime === void 0) ignoreBeforeTime = -_bigNum$2;
	...
	_proto2.totalTime = function totalTime(_totalTime2, suppressEvents) {
		if (!arguments.length) return this._tTime;
		this._forcing = 1;
		if (!this._dp && this._ts) this._start = _roundPrecise$1(_ticker.time - (this._ts > 0 ? _totalTime2 / this._ts : (this.totalDuration() - _totalTime2) / -this._ts));
...
var _win$2, _doc$2, _docElement, _pluginInitted, _tempDiv, _recentSetterPlugin, _reverting, _windowExists$1 = function _windowExists() {
	return typeof window !== "undefined";
}, _transformProps = {}, _RAD2DEG$1 = 180 / Math.PI, _DEG2RAD$1 = Math.PI / 180, _atan2$1 = Math.atan2, _bigNum$1 = 1e8, _capsExp$1 = /([A-Z])/g, _horizontalExp = /(left|right|width|margin|padding|x)/i, _complexExp = /[\s,\(]\S/, _propertyAliases = {
	...
	cache = parent._gsap;
	if (cache && toPercent && cache.width && horizontal && cache.time === _ticker.time && !cache.uncache) return _round$3(curValue / cache.width * amount);
	else {
	...
	var cap = 360, isString = _isString$2(endValue), change = parseFloat(endValue) * (isString && ~endValue.indexOf("rad") ? _RAD2DEG$1 : 1) - startNum, finalValue = startNum + change + "deg", direction, pt;
	if (isString) {
...
180 / Math.PI;
var _sin = Math.sin, _cos = Math.cos, _abs$1 = Math.abs, _sqrt = Math.sqrt, _isNumber$1 = function _isNumber(value) {
	return typeof value === "number";
...
function rawPathToString(rawPath) {
	...
	for (s = 0; s < l; s++) {
		...
		sl = segment.length;
		for (i = 2; i < sl; i++) result += _round$2(segment[i++]) + "," + _round$2(segment[i++]) + " " + _round$2(segment[i++]) + "," + _round$2(segment[i++]) + " " + _round$2(segment[i++]) + "," + _round$2(segment[i]) + " ";
...
* @author: Jack Doyle, jack@greensock.com
*/
var gsap$2, _coreInitted$2, _getGSAP$2 = function _getGSAP() {
...
}, _bigNum = 0x56bc75e2d63100000, _round$1 = function _round(value) {
...
var CustomEase = /* @__PURE__ */ function() {
	...
	CustomEase.getSVGData = function getSVGData(ease, config) {
		config = config || {};
		var width = config.width || 100, height = config.height || 100, x = config.x || 0, y = (config.y || 0) + height, e = gsap$2.utils.toArray(config.path)[0], a, slope, i, inc, tx, ty, precision, threshold, prevX, prevY;
		if (config.invert) {
			height = -height;
			y = 0;
		}
		if (typeof ease === "string") ease = gsap$2.parseEase(ease);
...
//#endregion
//#region src/components/effects/gsap/gsap.js
gsapWithCSS.registerPlugin(ScrollTrigger, CustomEase, CSSPlugin);
function heroIntroInit() {
	if (!window.matchMedia("(min-width: 767.98px)").matches) return;
	gsapWithCSS.registerPlugin(CustomEase);
	if (!gsapWithCSS.parseEase("mainEase")) CustomEase.create("mainEase", "0.18, 0.92, 0.34, 1");
	const hero = document.querySelector(".hero");
	const heroTitle = document.querySelector(".hero__title");
	const bottomTitle = document.querySelector(".bottom-hero__title--pc");
	const portfolioWidget = document.querySelector(".hero__portfolio-widget");
	const bottomMessage = document.querySelector(".bottom-hero__message");
	const slider = document.querySelector(".hero__slider");
	const headerMenu = document.querySelector(".header__menu--main");
	if (!heroTitle || !bottomTitle) return;
	const heroChars = heroTitle.querySelectorAll(".char");
	const bottomChars = bottomTitle.querySelectorAll(".char");
	const heroRect = hero.getBoundingClientRect();
	const heroCenter = heroRect.top + heroRect.height / 2;
	const heroTitleRect = heroTitle.getBoundingClientRect();
	const bottomTitleRect = bottomTitle.getBoundingClientRect();
	const heroOffset = heroCenter - (heroTitleRect.top + heroTitleRect.height / 2);
	const bottomOffset = heroCenter - (bottomTitleRect.top + bottomTitleRect.height / 2);
	gsapWithCSS.set(heroChars, {
		y: heroOffset,
		opacity: 0
	});
	gsapWithCSS.set(bottomChars, {
		y: bottomOffset,
		opacity: 0
	});
	gsapWithCSS.set(slider, { opacity: 0 });
	gsapWithCSS.set([portfolioWidget, bottomMessage], {
		opacity: 0,
		y: 20
	});
	if (headerMenu) gsapWithCSS.set(headerMenu, {
		opacity: 0,
		y: -20,
		flex: 0
	});
	gsapWithCSS.timeline({ delay: .3 }).to(heroChars, {
		opacity: 1,
		duration: .8,
		stagger: .12,
		ease: "mainEase"
		...
		stagger: .12,
		ease: "mainEase"
	}, "<0.6").to({}, { duration: .8 }).to(heroChars, {
		y: 0,
		duration: 1.8,
		ease: "mainEase"
	}).to(bottomChars, {
		...
		gsapWithCSS.set([heroChars, bottomChars], { clearProps: "transform,opacity" });
		if (headerMenu) gsapWithCSS.set(headerMenu, { clearProps: "opacity,transform,flex" });
	});
}
function heroScroll() {
	ScrollTrigger.matchMedia({ "(min-width: 767.98px)": function() {
		const portfolioWidget = document.querySelector(".hero__portfolio-widget");
		const bottomMessage = document.querySelector(".bottom-hero__message");
		if (!portfolioWidget || !bottomMessage) return;
		const elements = [portfolioWidget, bottomMessage];
...
function heroProjectInit() {
	const hero = document.querySelector(".hero-project__wrapper");
	if (!hero) return;
	gsapWithCSS.set(hero, {
...
function scrollText() {
	const sections = document.querySelectorAll(".introduction");
	if (!sections.length) return;
	sections.forEach((section) => {
		const words = section.querySelectorAll(".word");
		if (!words.length) return;
		gsapWithCSS.fromTo(words, { color: "#000" }, {
			color: "#fff",
			stagger: .01,
			ease: "none",
			scrollTrigger: {
...
function projectsShow() {
	...
	items.forEach((item) => {
		gsapWithCSS.fromTo(item, { opacity: .3 }, {
			opacity: 1,
			scrollTrigger: {
				trigger: item,
				start: "top 80%",
...
function scaleImage() {
	const bg = document.querySelector(".book-call__bg");
	if (!bg) return;
	gsapWithCSS.fromTo(bg, { scale: 1 }, {
		scale: 2,
		scrollTrigger: {
			trigger: ".book-call",
			start: "top bottom",
...
function morphText() {
	const el = document.querySelector(".action__text");
	if (!el) return;
	const phrases = [
		"Have a project?",
		"Got an idea?",
	...
	function loop() {
		...
		morphTo(phrases[currentIndex]);
		gsapWithCSS.delayedCall(3, loop);
	}
	gsapWithCSS.delayedCall(3, loop);
}
function buttonHover() {
	...
	function onEnter(e) {
		...
		gsapWithCSS.fromTo(btn, { "--r": "0px" }, {
			"--r": `${maxR}px`,
			duration: .6,
			ease: "power3.out",
			overwrite: true
		});
		if (span) gsapWithCSS.to(span, {
			color: "var(--black-100)",
			duration: .6,
			ease: "power3.out"
		});
	...
	requestAnimationFrame(() => {
		[
			...
			try {
				...
				console.error(`[gsap] Eror in ${fn.name}:`, e);
			}
		});
	});
}
document.querySelector("[data-fls-gsap]") && window.addEventListener("load", gsapInit);
//#endregion
